...
try {
  ...
  // 🔍 Detecta modo de autenticação
  if (process.env.FIREBASE_CREDENTIALS_B64) {
    console.log("📦 Usando credenciais Base64 (variável de ambiente).");
    creds = JSON.parse(Buffer.from(process.env.FIREBASE_CREDENTIALS_B64, "base64").toString());
  } else if (fs.existsSync("./serviceAccountKey.json")) {
    console.log("📁 Usando arquivo local serviceAccountKey.json.");
    creds = JSON.parse(fs.readFileSync("./serviceAccountKey.json", "utf8"));
  } else {
    throw new Error("Nenhuma credencial Firebase encontrada! Defina FIREBASE_CREDENTIALS_B64 ou adicione serviceAccountKey.json.");
  }
